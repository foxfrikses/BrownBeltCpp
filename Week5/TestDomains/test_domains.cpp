...
void ExtraEmptyStringWhileSplitTest() {
  auto res = Split("hello.world", ".");
  
  vector<string_view> expected {"hello", "world"};
  
  ASSERT_EQUAL(res, expected);
}

void GetReversedPartsTest() {
  Domain domain("hello.world");
  
  vector<string_view> result (domain.GetReversedParts().begin(),
                              domain.GetReversedParts().end());
  vector<string_view> expected {"world", "hello"};
  
  ASSERT_EQUAL(result, expected);
}

void NotSelfSubdomenTest() {
  Domain domain("hello.world");
  ASSERT(IsSubdomain(domain, domain));
}

void ReversedDomenSubdomenOrderTest() {
  Domain subdomain("hello.world");
  Domain domain("world");
...
void GoodBadMisplacedTest() {
  vector<bool> results {true, true, false, true};
  stringstream ss;
  PrintCheckResults(results, ss);
  ASSERT_EQUAL("Good\nGood\nBad\nGood\n", ss.str());
}

void IncorrectSuccessLogicTest() {
  vector<Domain> banned {
    Domain("heh")
  };
  vector<Domain> domainsToCheck {
    Domain("sgd.heh"),
    Domain("sgd.he"),
  };
  const vector<bool> res = CheckDomains(banned,domainsToCheck);
  const vector<bool> expected{false, true};
  ASSERT_EQUAL(res, expected);
}

void IncorrectGetlineUseTest() {
  stringstream ss("1\nheh");
  auto domains = ReadDomains(ss);
  ASSERT_EQUAL(domains.front().GetPartCount(), 1);
}

void DomainCheckerAbsorbTest() {
  vector<Domain> domains {Domain("a"), Domain("a.a")};
  Domain domain("b.a");
  
  DomainChecker chacker(domains.begin(), domains.end());
  ASSERT(chacker.IsSubdomain(domain));
}

int main() {
  TestRunner tr;
  RUN_TEST(tr, ExtraEmptyStringWhileSplitTest);
  RUN_TEST(tr, GetReversedPartsTest);
  RUN_TEST(tr, NotSelfSubdomenTest);
  RUN_TEST(tr, ReversedDomenSubdomenOrderTest);
  RUN_TEST(tr, GoodBadMisplacedTest);
  RUN_TEST(tr, IncorrectSuccessLogicTest);
  RUN_TEST(tr, IncorrectGetlineUseTest);
  RUN_TEST(tr, DomainCheckerAbsorbTest);
