...
public:
  ...
  class ValueWriter final {
  public:
    ValueWriter(vector<T>& v, size_t index)
      : data(v)
      , index(index)
    {}
    operator T() const {
    ...
    ValueWriter& operator=(T value) {
      if (!value) {
        if (index == data.size() - 1) {
          auto it = std::find_if(next(data.rbegin()), data.rend(), [] (T value) {return value;});
          data.resize(data.rend() - it);
          return *this;
        }
  ...
  ValueWriter operator [](size_t degree) {
    return ValueWriter(coeffs_, degree);
  }
...
void TestDecreaseDegreeAfterZeroAssign() {
  Polynomial<int> poly;
...
int main() {
  ...
  RUN_TEST(tr, TestDecreaseDegreeAfterZeroAssign);
