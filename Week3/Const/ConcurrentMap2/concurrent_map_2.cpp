...
public:
  struct ReadAccess {
    ReadAccess(const MapMutex& m, const K& key)
        : lock(m._mutex)
        , ref_to_value(m._map.at(key))
    {}
  ...
  struct WriteAccess {
    WriteAccess(MapMutex& m, const K& key)
      : lock(m._mutex)
      , ref_to_value(m._map[key])
    {}
  ...
  WriteAccess operator[](const K& key) {
    size_t bucket = hasher(key) % maps.size();
    return WriteAccess(maps[bucket], key);
  }
  ReadAccess At(const K& key) const {
    size_t bucket = hasher(key) % maps.size();
    return ReadAccess(maps.at(bucket), key);
  }
  bool Has(const K& key) const {
    size_t bucket = hasher(key) % maps.size();
    auto& m = maps[bucket];
    lock_guard lock(m._mutex);
    return m._map.find(key) != m._map.end();
  }
  ...
  MapType BuildOrdinaryMap() const {
    ...
    for (auto& m : maps) {
      lock_guard lock(m._mutex);
      res.insert(m._map.begin(), m._map.end());
    }
    return res;
  }

private:
  vector<MapMutex> maps;
  Hash hasher;
};
